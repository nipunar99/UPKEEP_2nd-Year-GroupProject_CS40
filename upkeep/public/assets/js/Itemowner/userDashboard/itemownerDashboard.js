...
// const targetDate = new Date(2023, 5, 13); // Note: Months are zero-based, so May is represented by 4
...
const renderCalendar = (targetDate) => {
  ...
  for (let i = 1; i <= lastDateOfMonth; i++) {
    ...
    let isTargetDate =
      targetDate &&
      i === targetDate.getDate() &&
      currMonth === targetDate.getMonth() &&
      currYear === targetDate.getFullYear()
        ? "active"
        : "";
    let isToday =
      i === date.getDate() &&
      ...
      currYear === new Date().getFullYear()
        ? "active"
        : "";
    liTag += `<li class="${isTargetDate || isToday}">${i}</li>`;
  }
...
const targetDate = new Date(2023,6, 1); // Note: Months are zero-based, so May is represented by 4
renderCalendar(targetDate);
